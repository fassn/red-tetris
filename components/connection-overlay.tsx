...
const ConnectionOverlay = ({ status, error }: ConnectionOverlayProps) => {
    ...
    return (
        <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/50' role='dialog' aria-modal='true' aria-label='Connection status'>
            <div className='bg-surface-card rounded-lg shadow-xl p-8 mx-4 max-w-sm text-center' role='alert' aria-live='assertive'>
                {status === 'connecting' && (
                    <>
                        ...
                        <p className='text-content-secondary text-sm mt-1'>Please wait</p>
                    </>
                )}
                {status === 'disconnected' && (
                    <>
                        ...
                        <p className='text-content-secondary text-sm mt-1'>Reconnecting…</p>
                    </>
                )}
                {status === 'error' && (
                    <>
                        ...
                        <p className='text-content-secondary text-sm mt-1'>{error || 'Unable to reach the server'}</p>
                        <button
                            onClick={() => window.location.reload()}
                            className='mt-4 px-6 py-2 bg-brand rounded-sm font-semibold uppercase hover:bg-brand-hover hover:text-content-inverse transition-colors focus:outline-hidden focus:ring-2 focus:ring-brand focus:ring-offset-2'
                        >
                            Retry
                        </button>
                    </>
